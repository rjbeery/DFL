...
class SubprocessGenerator(object):
    ...
    def process_func(self, user_param):
        try:
            ...
            import traceback
            self.cs_queue.put( Exception(f"SubprocessGenerator worker crashed:\n{traceback.format_exc()}") )

    def _check_for_worker_error(self, data):
    ...
    def __next__(self):
        ...
        while True:
            try:
                gen_data = self.cs_queue.get(timeout=30)
                break
            except Exception:
                if not self.p.is_alive():
                    raise RuntimeError(
                        f"SubprocessGenerator worker (pid={self.p.pid}) died silently "
                        f"(exit code {self.p.exitcode}). Check for DLL/import errors in the worker."
                    )
                # still alive, just slow — keep waiting
        if gen_data is None:
            self.p.terminate()
            self.p.join()
            raise StopIteration()
        self._check_for_worker_error(gen_data)
        self.sc_queue.put (1)
        return gen_data
