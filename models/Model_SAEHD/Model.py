...
""").lower()
    ...
    def on_initialize(self):
        ...
        if self.is_training:
            ...
            # Cap at 2 workers per generator. More than ~2 causes Windows to
            # establish too many named-pipe connections simultaneously (each
            # SubprocessGenerator pickle carries all sibling queues), which
            # stalls p.start() indefinitely on Windows with the spawn method.
            src_generators_count = min(cpu_count // 2, 2)
            dst_generators_count = min(cpu_count // 2, 2)
            if ct_mode is not None:
                src_generators_count = min(int(src_generators_count * 1.5), 2)
